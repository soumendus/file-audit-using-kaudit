...
static int audit_failure_parser(const char *val, int line)
{
	...
	for (i=0; failure_actions[i].name != NULL; i++) {
		if (strcasecmp(val, failure_actions[i].name) == 0) {
			config.failure_action = (auditfail_t)failure_actions[i].option; // Add cast (auditfail_t) by soumendu
			return 0;
		}
	}
...
int audit_update_watch_perms(struct audit_rule_data *rule, int perms)
{
	...
	for (i=0; (unsigned)i< rule->field_count; i++) {
			...
			const char *path)
{
	...
	*rulep = (struct audit_rule_data*)realloc(rule, len + sizeof(*rule)); // Added struct audit_rule_data by soumendu
	if (*rulep == NULL) {
		free(rule);
			 ...
			 const char *subtree)
{
	...
	struct cmd *pcmd; // Added by soumendu

	cout<<"inside audit_make_equivalent()"<<endl;
...
#endif
 	pcmd = (struct cmd*)malloc(sizeof(struct cmd) + len1 + len2);
	...
	free(cmd);
#endif
 	memset(pcmd, 0, sizeof(pcmd) + len1 + len2);

 	pcmd->sizes[0] = len1;
 	pcmd->sizes[1] = len2;
 	memcpy(&pcmd->buf[0], mount_point, len1);
 	memcpy(&pcmd->buf[len1], subtree, len2);

 	rc = audit_send(fd, AUDIT_MAKE_EQUIV, pcmd, sizeof(*pcmd) + len1 + len2);
	if (rc < 0) 
		audit_msg(audit_priority(errno),
			"Error sending make_equivalent command (%s)",
			strerror(-rc));
...
uid_t audit_getloginuid(void)
{
	...
	close(in);
	if (len < 0 || (unsigned)len >= sizeof(buf)) // (unsigned) added by soumendu
		return -1;
	buf[len] = 0;
	errno = 0;
	uid = strtol(buf, 0, 10);
                              ...
                              int flags)
{
	...
	*/
	if ( (v = strstr((char*)pair, "!=")) ) { // Added (char*) by soumendu
		*v++ = '\0';
		*v++ = '\0';
		op = AUDIT_NOT_EQUAL;
	} else if ( (v = strstr((char*)pair, ">=")) ) { 
		*v++ = '\0';
		*v++ = '\0';
	...
	switch (field)
	{
		...
		case AUDIT_FILTERKEY:
			...
			offset = rule->buflen;
			rule->buflen += vlen;
			*rulep = (struct audit_rule_data*)realloc(rule, sizeof(*rule) + rule->buflen); // Added struct audit_rule_data by soumendu
			if (*rulep == NULL) {
				free(rule);
				audit_msg(LOG_ERR, "Cannot realloc memory!\n");
				return -3;
