...
int audit_get_reply(int fd, struct audit_reply *rep, reply_t block, int peek)
{
	...
	if (block == GET_REPLY_NONBLOCKING)
		block = MSG_DONTWAIT; // Commented By Soumendu
#endif
...
#endif
	len = recvfrom(fd, &rep->msg, sizeof(rep->msg), MSG_DONTWAIT|peek,
		(struct sockaddr*)&nladdr, &nladdrlen);
...
static int adjust_reply(struct audit_reply *rep, int len)
{
	...
	switch (rep->type) {
		case NLMSG_ERROR: 
			rep->error   = (nlmsgerr*)NLMSG_DATA(rep->nlh); // Added (nlmsgerr*) by soumendu
			break;
		case AUDIT_GET:   
			rep->status  = (audit_status*)NLMSG_DATA(rep->nlh); // Added (audit_status*) by soumendu
			break;
		case AUDIT_LIST_RULES:  
			rep->ruledata = (audit_rule_data*)NLMSG_DATA(rep->nlh); // Added (audit_rule_data*) by soumendu
			break;
		case AUDIT_USER:
		case AUDIT_LOGIN:
		case AUDIT_KERNEL:
		...
		case AUDIT_FIRST_EVENT...AUDIT_INTEGRITY_LAST_MSG:
			rep->message = (const char*)NLMSG_DATA(rep->nlh); // Added (const char*) by soumendu
			break;
		case AUDIT_SIGNAL_INFO:
			rep->signal_info = (audit_sig_info*)NLMSG_DATA(rep->nlh); // Added (audit_sig_info*) by soumendu
			break;
	}
	return len;
}
